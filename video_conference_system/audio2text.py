...
class Speech_to_Text:
    ...
    def __call__(self, file_path):
        ...
        if self.isVideo(self.file_path):
            ...
            raise TypeError(
                "Cannot handle this file format. Must be video(.avi, .mov, .mp4, .ogv, .webm) or audio(.mp3, .ogg, .wav, .flv, .raw)"
            )
    ...
    def breakdown(self, segment_length):
        ...
        for start_sec in range(0, self.duration, segment_length):
            ...
            link_segment = self.audio[
                (start_sec + segment_length / 2)
                * _ms : (start_sec + 3 * segment_length / 2)
                * _ms
            ]
    ...
    def isVideo(self, filename):
        if filename.endswith((".avi", ".mov", ".mp4", ".ogv", ".webm")):
            return True
        else:
            return False
